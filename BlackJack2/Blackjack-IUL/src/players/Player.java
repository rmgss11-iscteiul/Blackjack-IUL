...
public abstract class Player extends Person {
	...
	private int money, splitPoints;
	private int bet, splitMoney;
	private Dealer dealer;
	private boolean playFinish, splitHandFinish, handSplited;
	private ArrayList<Card> splitHand;
	//	private boolean lost; //apagar lost
	public enum loseDrawWin {WIN, DRAW, LOSE;}
	...
	public void bet() {// sinto que devia estar aqui alguma coisa
	}

	public void hit() {
		if(!splitHandFinish && handSplited && splitPoints<21) {
	...
	public void stand() {
		if(!splitHandFinish && handSplited)
			splitHandFinish=true;
		else
			playFinish=true;
	}
	...
	public void doubleBet() {		
		if(bet!=0 && money-bet>0 && hand.size()==2) {
			bet*=2;
			hit();
			try {
		...
		if(twoEqualCards(super.getHand())!=-1 && money-bet>0 && super.getHand().size()==2 && !handSplited) {
			...
			if(twoEqualCards(super.getHand())==1) {
				...
				if(super.getHand().get(1).number==Number.TEN||super.getHand().get(1).number==Number.JACK
						||super.getHand().get(1).number==Number.QUEEN || super.getHand().get(1).number==Number.KING){
					money+=bet*2;
					playFinish = true;
					win();
				}
	...
	private void countSplitPoints(ArrayList<Card> list) {
		if(hand.size()!=0) {
			...
			for(Card card: splitHand) {
				if(card.number == Card.Number.JACK || card.number == Card.Number.QUEEN || card.number == Card.Number.KING) {
					splitPoints+=10;}
